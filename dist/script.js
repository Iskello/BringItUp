...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_accordeon__WEBPACK_IMPORTED_MODULE_5__["default"]('.module__info-show').init();
    ...
    value: function init() {
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          var hideContent = btn.nextElementSibling;
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      ...
      var setCursorPosition = function setCursorPosition(pos, elem) {
        elem.focus(); //перевіряє чи є метод та встановлює діапазон виділення тексту

        if (elem.setSelectionRange) {
          elem.setSelectionRange(pos, pos); //перевіряємо інший метод для старіших версій IE
  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            ...
            _this.activeBtn = btn; //Ця умова виконається тільки тоді, коли ми вже 1 раз викликали наш плеєр - тоді у блока iframe буде id frame

            if (document.querySelector('iframe#frame')) {
              _this.overlay.style.display = 'flex'; //виправлення багу, що відкривається одне і те ж саме відео
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //копіюємо повністю svg активного елемента

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //перевіряємо стан відео на завершеність state.data === 0
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      } catch (e) {} //ховаємо всі непотрібні слайди
      ...
      this.prev.addEventListener('click', function () {
        ...
        for (var i = _this3.slides.length - 1; i > 0; i--) {
          if (_this3.slides[i].tagName !== 'BUTTON') {
            ...
            var active = _this3.slides[i]; //ставимо останній слайд в початок (1-й аргумент - елемент, 2-й - куди ставимо)
    ...
    value: function init() {
      ...
      try {
        ...
        if (this.autoplay) {
          ...
          }, 5000); // Зберігаємо інтервал в змінній
        }

        this.container.addEventListener('mouseover', function () {
          clearInterval(_this4.interval); // Зупиняємо інтервал при наведенні на контейнер
        });
        this.container.addEventListener('mouseout', function () {
          if (_this4.autoplay) {
            _this4.interval = setInterval(function () {
              return _this4.nextSlide();
            }, 5000); // Поновлюємо інтервал при знятті курсора з контейнера
          }
        });
        this.next.addEventListener('mouseover', function () {
          clearInterval(_this4.interval); // Зупиняємо інтервал при наведенні на кнопки
        });
        this.next.addEventListener('mouseout', function () {
          if (_this4.autoplay) {
            _this4.interval = setInterval(function () {
              return _this4.nextSlide();
            }, 5000); // Поновлюємо інтервал при знятті курсора з кнопки
          }
        });
        this.prev.addEventListener('mouseover', function () {
          clearInterval(_this4.interval); // Зупиняємо інтервал при наведенні на кнопки
        });
        this.prev.addEventListener('mouseout', function () {
          if (_this4.autoplay) {
            _this4.interval = setInterval(function () {
              return _this4.nextSlide();
            }, 5000); // Поновлюємо інтервал при знятті курсора з кнопки
